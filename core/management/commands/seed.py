...
class Command(BaseCommand):
    ...
    def _load_items(self):
        ...
        item_table = pd.read_html(io=StringIO(html), match="Master Ball")[0].drop(index=[0, 1, 3], axis=1)
        item_table.columns = ['id', 'hex_id', 'bag_sprite', 'item_description']

        item_table = item_table.drop(index=item_table[item_table["item_description"].str.contains("unknown", regex=False)].index)
        item_table = item_table.drop(index=item_table[item_table['item_description'].str.contains("Nothing", regex=False)].index)
        item_table = item_table.drop(index=item_table[item_table['id'].str.contains("#", regex=False)].index)

        new_items = item_table.apply(lambda r: (
            Item(
                id=int(r['id']),
