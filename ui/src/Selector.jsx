  ...
  return (
    <>
      ...
      <select
        id={id}
        onChange={onChange ?? (() => 1)}
        class={"shadow-sm border border-solid border-gray-400 bg-white p-1 rounded-md hover:outline hover:outline-2 hover:outline-solid hover:outline-black hover:cursor-pointer " + className}
      >
        <For each={options()}>
          {({ value, label }) => (
            <option
