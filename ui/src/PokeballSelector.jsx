...
function ClickablePokeball({ value, onClick, selected }) {
  ...
  const className = () => {
    if (selected()) {
      return 'h-8 w-8 rounded-full outline outline-2 outline-solid outline-black sharp-pixels';
    }
    return 'h-8 w-8 rounded-full hover:outline hover:outline-2 hover:outline-dotted hover:outline-black sharp-pixels';
  };
  return (
